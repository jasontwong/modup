...
class Cache
{
    ...
    public static function set($key, $value, $time=0, $namespace=NULL)
    {
        try
        {
            ...
            {
                $q->andWhere('namespace IS NULL');
            }
            else
            {
                $q->andWhere('namespace = ?', $namespace);
            }
            $data_rows = $q->execute();
            $rows = $data_rows->count();
            ...
            if ($rows)
            {
                $u = Doctrine_Query::create()
                        ->update('CacheData')
                        ->set('data', '?', $value)
                        ->set('expire', '?', $time === 0 ? 0 : time() + $time)
                        ->where('name = ?', $key);
                if (is_null($namespace))
                {
                    $u->andWhere('namespace IS NULL');
                }
                else
                {
                    $u->andWhere('namespace = ?', $namespace);
                }
                $result = $u->execute();
                return $result !== 0;
            }
            else
            {
                ...
                $cache->expire = $time === 0 ? 0 : time() + $time;
                $cache->save();
                return TRUE;
            }
        }
    ...
    public static function remove($key, $seconds=0, $namespace=NULL)
    {
        try
        {
            ...
            else
            {
                $r->update('CacheData')
                    ->set('expire', '?', time() - 1)
                    ->set('lockout', '?', time() + $seconds);
            }
            if (is_null($namespace))
            {
                $r->andWhere('namespace IS NULL');
            }
            else
            {
                $r->andWhere('namespace = ?', $namespace);
            }
            $rows = $r->execute();
            return $rows ? self::REMOVE_ITEM_MISSING : self::REMOVE_SUCCESSFUL;
        }
        catch (Exception $e)
        {
