...
function modup_autoload($class)
{
    $file = DIR_SYS.'/'.strtolower($class).'.php';
    if (substr($class, 0, 11) === 'FormBuilder')
    {
        include_once DIR_SYS.'/form.php';
    }
    elseif (!class_exists($class) && is_file($file))
    {
        include_once DIR_SYS.'/'.strtolower($class).'.php';
    }
}
//}}}
...
function test_db_settings($db)
{
    try
    {
        $dbc = new PDO($db['type'].':host='.$db['host'].';dbname='.$db['name'], $db['user'], $db['pass']);
        $dbc = NULL;
        return TRUE;
    }
...
//}}}
    //{{{ function dql_build($specs)
    /**
     * Build out a Doctrine_Query object via array
     *
     * The array must be dql_method => parameters. Non DQL methods in the array
     * will be ignored.
     */
    function dql_build($specs)
    {
        $dql = Doctrine_Query::create();
        foreach ($specs as $method => $param)
        {
            if (method_exists($dql, $method))
            {
                $lc_method = strtolower($method);
                if (strpos($lc_method, 'where') === 0 && is_array($param))
                {
                ...
                elseif ((strpos($lc_method, 'or') === 0 || strpos($lc_method, 'and') === 0 || strpos($lc_method, 'add') === 0) && strpos($lc_method, 'where') !== FALSE && is_array($param))
                {
                    foreach ($param as $v)
                    {
                        if (is_array($v))
                ...
                elseif ($method === 'set' && is_array($param))
                {
                    foreach ($param as $k => $v)
                    {
                        $dql->$method($k, '?', is_array($v) && empty($v) ? array('') : $v);
                    }
                }
                else
                {
                    $dql->$method(is_array($param) ? implode(',', $param) : $param);
                }
            }
        }
        return $dql;
    ...
    //}}}
    //{{{function dql_exec($specs, $params = array())
    /**
     * Build out a Doctrine_Query object via array
     *
     * The array must be dql_method => parameters. Non DQL methods in the array
    ...
    {
        $dql = dql_build($specs);
        return $dql->execute($params, Doctrine_Core::HYDRATE_ARRAY);
    }
    //}}}
...
//}}}
// {{{ function export_csv($data, $col_headers = array(), $return_string = false)
function export_csv($data, $col_headers = array(), $return_string = false)
{
