...
$coupons = array_merge(Ecommerce::get_available_coupons(), Ecommerce::get_expired_coupons());
$options = array();
...
foreach ($eo->Products as $k => $product)
{
    ...
    foreach ($product->Options as $option)
    {
        ...
        if (strlen($option->id))
        {
            $values['_'.$option->name.'_product_id'][$k][] = $product->id;
        }
...
foreach ($eo->Options as $option)
{
    ...
    if (strlen($option->id))
    {
        $values[$option->name.'_order_id'] = $eo->id;
    }
...
foreach (Ecommerce::get_order_keys() as $key)
{
    ...
    if (ake($key.'_order_id', $values))
    {
        $layout->add_layout(
            array(
                'field' => Field::layout('hidden'),
                'name' => '_'.$key.'_order_id',
                'type' => 'hidden',
                'hidden' => TRUE,
                'value' => array(
                    'data' => $values[$key.'_order_id'],
                ),
            )
        );
        $row = array();
        $row['row']['attr']['class'] = 'row_hidden';
        $row['fields'] = $layout->get_layout('_'.$key.'_order_id');
        $rows[] = $row;
    }
    unset($row);
}
...
if (isset($_POST['form']))
{
    ...
    if (ake('return', $form))
    {
        header('Location: /admin/module/Ecommerce/return_order/'.URI_PART_4.'/');
        exit;
    }
    if (ake('delete', $form))
    {
    ...
    foreach ($options as $k => $v)
    {
        if (strpos($k, '_') === 0)
        {
            if (strpos($k, '_order_id') !== FALSE && strlen($k) > 3)
            {
                if (strlen($v))
                {
                    $order['Options'][str_replace('_order_id', '', substr($k, 1))]['order_id'] = $v;
                }
            }
            elseif (strpos($k, '_id') !== FALSE && strlen($k) > 3)
            {
                if (strlen($v))
                {
    ...
    {
        foreach ($new_products as $k => &$prod)
        {
            if ($prod['id'] === $product->id)
            {
                ...
                foreach ($product->Options as &$option)
                {
                    foreach ($prod['Options'] as $n => &$opt)
                    {
                        if (ake('id', $opt) && $opt['id'] === $option['id'])
                        {
                            $options[] = $opt;
                            unset($prod['Options'][$n]);
                            break;
                        }
                    }
                }
                $prod['Options'] = array_merge($options, $prod['Options']);
                break;
            }
        }
    }
    ...
    {
        $eo->link('ReturnStatus', $data['return_status_id']);
    }
...
$eform->add_group(
    array(
        'rows' => array(
            ...
            array(
                'fields' => $layout->get_layout('return_status_id'),
                'label' => array(
                    'text' => 'Return Status'
                )
            ),
...
if ($eor === FALSE)
{
    $eform->add_group(
        array(
            'rows' => array(
                array(
                    'fields' => $layout->get_layout('return'),
                ),
            ),
        ),
        'form'
