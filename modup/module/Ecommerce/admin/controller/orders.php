...
$filter['return_statuses'] = array();

User::update('setting', 'ecommerce', 'order_filter', $filter);
User::update('setting', 'ecommerce', 'order_filter', $filter);
...
if (ake('export', $_REQUEST))
{
    ...
    switch ($type)
    {
        case 'pdf':
            ...
            $filename = 'orders-'.time().'.pdf';
            $pdf = new FPDF('L', 'mm', array(841, 594));
            $pdf->AddPage();
            $pdf->SetAutoPageBreak(TRUE);
            $pdf->SetFont('Arial', 'B', 12);
            foreach ($columns as &$col)
            {
                $pdf->Cell(50, 20, $col, 0, 'L');
            }
            $pdf->Ln();
