...
class Inventory
{
    ...
    public function hook_admin_css()
    {
        ...
        if (strpos(URI_PATH, '/admin/module/Inventory/') === 0)
        {
            $css['screen'][] = '/admin/static/Inventory/css/admin.css/';
        }
        return $css;
    }
    //}}}
    ...
    // {{{ static public function deduct_quantity($product_id, $option_x, $option_y, $qty)
    static public function deduct_quantity($product_id, $option_x, $option_y, $qty)
    {
        $quantities = self::get_quantities($product_id);
        $pq = deka(NULL, $quantities, $option_x, $option_y);
        if (is_numeric($pq['id']))
        {
            $quantity = $pq['quantity'] - $qty;
            $specs = array(
                'update' => 'InventoryProductQuantity pq',
                'set' => array(
                    'pq.quantity' => $quantity < 0 ? 0 : $quantity,
                ),
                'where' => array(
                    'pq.id = ?' => $pq['id'],
                ),
    ...
    //}}}
    // TODO adjust is same as deduct, remove
    // {{{ static public function adjust_quantity($product_id, $option_x, $option_y, $qty)
    static public function adjust_quantity($product_id, $option_x, $option_y, $qty)
    {
        $quantities = self::get_quantities($product_id);
        $pq = deka(NULL, $quantities, $option_x, $option_y);
    ...
    //}}}
    // {{{ static public function add_quantity($product_id, $option_x, $option_y, $qty)
    static public function add_quantity($product_id, $option_x, $option_y, $qty)
    {
        $quantities = self::get_quantities($product_id);
        $pq = deka(NULL, $quantities, $option_x, $option_y);
    ...
    static public function get_product_group($id)
    {
        ...
        return array_pop($product_groups);
    }
    //}}}
    ...
    static public function get_product($id)
    {
        ...
        /**
         * inventory array should have data to match the options grid even if 
         * it is an empty string, so the javascript can easily loop through
         * the data.
         */
        $count_x = count($product['options_x']);
        $count_y = count($product['options_y']);
        for ($y = 0; $y < $count_y; $y++)
        {
            for ($x = 0; $x < $count_x; $x++)
            {
                if (!eka($product, 'inventory', $y, $x))
                {
                    $product['inventory'][$y][$x] = '';
                }
            }
        }
    ...
    //}}}
    // TODO swap get_all_quantities with get_quantities for repo version
    //{{{ static public function get_all_quantities()
    static public function get_all_quantities()
    {
        // options
        $specs = array(
            ...
            'from' => 'InventoryProductOption po',
            'leftJoin' => array(
                'po.InventoryOption o',
                'o.InventoryOptionGroup g',
            ),
            'orderBy' => 'po.axis ASC, po.weight ASC'
        );
        $options = dql_exec($specs);
        $products = array();
        foreach ($options as $option)
        {
            $axis = $option['axis'];
            ...
            unset($option['axis']);
            */
            $products[$option['inventory_product_id']][$axis][$option['weight']] = $option;
            // $product[$key][] = $option;
            // $product[$axis][] = $option['id'];
        }
        ...
        $specs = array(
            ...
            'from' => 'InventoryProductQuantity pq',
        );
        $data = dql_exec($specs);
        foreach ($data as &$v)
        {
            $product = $products[$v['inventory_product_id']];
            $v['option_x'] = $product['x'][$v['weight_x']];
            $v['option_y'] = $product['y'][$v['weight_y']];
        }
        return $data;
    }
    //}}}
    // TODO turn get_quantities to get_quantities_by_id
    //{{{ static public function get_quantities($product_id)
    static public function get_quantities($product_id)
    {
        // options
        $specs = array(
        ...
        $options = dql_exec($specs, array($product_id));
        $product['options_x'] = array();
        $product['options_y'] = array();
        $product['x'] = array();
        $product['y'] = array();
        ...
        $specs = array(
            ...
            'from' => 'InventoryProductQuantity pq',
            'addWhere' => array(
                'pq.inventory_product_id = ?',
            ),
        );
        $data = dql_exec($specs, array($product_id));
        $quantities = array();
        foreach ($data as $v)
        {
            $quantities[$product['x'][$v['weight_x']]][$product['y'][$v['weight_y']]] = $v;
        }
        return $quantities;
    }
    //}}}
    // {{{ static public function get_quantity($product_id, $option_x, $option_y)
    static public function get_quantity($product_id, $option_x, $option_y)
    {
        $quantity = self::get_quantities($product_id);
        return deka(NULL, $quantity, $option_x, $option_y, 'quantity');
    }
    //}}}
